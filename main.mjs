...
if(workerpool.platform==='browser'){processor.singleModeInit(statsFor('browserPlatformProcess'),thisPool);processor.singleMode()}
/*Single or multiple process+pool mode*/
	...
	/*Multiple process+pool mode*/
		...
		import('cluster').then((cluster)=>{
			...
			else if(cluster.isWorker){processor.multiModeInit(statsFor('clusterSpawnedProcess'),thisPool);processor.multiMode(cluster)}
